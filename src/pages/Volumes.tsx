              ...
              if (entry.type === "group") {
                ...
                return (
                  <Fragment key={`group-${entry.project}`}>
                    ...
                    {expandedGroups[entry.project] && entry.volumes.map((volume) => (
                      <tr key={volume.name} className="group border-b border-border/50 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="p-3 font-mono text-foreground flex items-center gap-2 pl-8"><HardDrive className="w-3.5 h-3.5 text-primary" /> {volume.name}</td>
